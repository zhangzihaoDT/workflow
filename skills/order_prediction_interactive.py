...
class OrderPredictor:
    ...
    def predict_cm2(self, target_days, current_day=None):
        ...
        # CM2节奏更快：21天达到28169单，相当于CM1的28天26937单
        # CM2的时间压缩比例：21/28 = 0.75，即CM2用75%的时间完成CM1的体量
        cm2_acceleration_ratio = 0.75  # CM2节奏加速比例
        ...
        if current_day <= initial_threshold and 'initial' in self.cm2_phase_models:
            ...
            if 'final' in self.cm2_phase_models and current_day <= acceleration_start:
                ...
                # 长期预测：根据目标天数调整增长策略
                last_actual = cm2_df['cumulative_orders'].iloc[-1]  # 28169
                last_day = cm2_df['day'].iloc[-1]  # 21
                
                if current_day > acceleration_start:
                    # 根据目标天数调整加速策略
                    days_beyond = current_day - last_day
                    
                    # CM2的基础日增长（基于实际数据：第16-21天平均增长）
                    cm2_recent_daily_growth = np.mean([1656, 1773, 864, 886, 730, 911])  # 约1137单/天
...
def create_interface():
    with gr.Blocks(title="CM2订单数预测工具", theme=gr.themes.Soft()) as demo:
        ...
        基于CM1的全量数据（28天）和CM2的部分数据（21天）进行**S型曲线**和**分段建模**预测。
