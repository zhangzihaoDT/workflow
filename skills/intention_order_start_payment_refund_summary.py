...
def summarize_for_period(
    ...
    # day1～3：包含 start 当天算 day1（区间：start ～ start+2 天）
    d1 = start
    d3 = start + pd.Timedelta(days=2)
    # 为了确保第3天整天被纳入统计，使用起始+3天的开区间上界
    d3_exclusive = start + pd.Timedelta(days=3)

    df_pay = df[~df[pay_col].isna()].copy()
    df_refund = df[~df[refund_col].isna()].copy()

    in_d1_d3_pay = df_pay[(df_pay[pay_col] >= d1) & (df_pay[pay_col] <= d3)]
    # 退订数定义更新：仅统计有支付时间且支付时间早于退订时间的订单在起始1～3天内发生的退订
    df_refund_paid = df_refund[~df_refund[pay_col].isna()].copy()
    # 退订时间窗口：起始日（含）到起始+3天（不含），等价于退订时间≤起始+2的整天
    mask_window = (df_refund_paid[refund_col] >= d1) & (df_refund_paid[refund_col] < d3_exclusive)
    ...
    g1 = in_d1_d3_pay.groupby(group_col).size().rename("payment_d1_d3")
    g2 = in_d1_d3_refund.groupby(group_col).size().rename("refund_d1_d3")
    ...
    # 新增比值列（百分比形式，保留一位小数）：refund_d1_d3 / payment_d1_d3
    ratio = (
        out["refund_d1_d3"].astype("Float64")
        / out["payment_d1_d3"].replace({0: pd.NA}).astype("Float64")
    )
    out["refund_to_payment_ratio_d1_d3"] = ratio.astype("Float64").mul(100).round(1).map(
...
def main():
    parser = argparse.ArgumentParser(description="按车型分组输出各业务周期起始第1-3天及全周期的支付/退订统计")
    ...
    for key, rng in periods.items():
        ...
        if pd.isna(start) or pd.isna(end):
            ...
            continue
        res = summarize_for_period(df, group_col, pay_col, refund_col, start, end)
        ...
        if res.empty:
            res = pd.DataFrame({
                ...
                "refund_to_payment_ratio_d1_d3": [pd.NA],
            })
